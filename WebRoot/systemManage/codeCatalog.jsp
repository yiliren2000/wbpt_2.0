...
<c:set var="entityName"
	value="com.lxq.platform.systemManage.pojo.CodeCatalog"></c:set>
<c:set var="entityName2"
	value="com.lxq.platform.systemManage.pojo.CodeLibrary"></c:set>
...
<script type="text/javascript">
	var params = {
		...
		/**grid 的sql from 语句**/
		gridSql : 'from basic_code_catalog',

		/**form 的sql from 语句**/
		formSql : 'from basic_code_catalog',
		...
		buttons : [ {
			...
			name : 'query_button',
			visible : ${pt:hasRight(curUser,'query', entityName)}
		}, {
			name : 'update_button',
	...
	var params2 = {
		...
		/**grid 的sql from 语句**/
		gridSql : 'from basic_code_catalog,basic_code_library where basic_code_library.codeCatalog = basic_code_catalog.uid',

		/**form 的sql from 语句**/
		formSql : 'from basic_code_library',

		/**排序字段**/
		sort : 'basic_code_library.uid',
		...
		gridFields : [ {
			...
			header : '代码值',
			name : 'value',
			field : 'basic_code_library.value',
			sortable : true,
			searchable : true
		}, {
			type : 'textfield',
		...
		buttons : [ {
			name : 'add_button',
			visible : ${pt:hasRight(curUser,'add', entityName2)}
		}, {
			name : 'query_button',
			visible : false
		}, {
			name : 'update_button',
			visible : ${pt:hasRight(curUser,'update', entityName2)}
		}, {
			name : 'delete_button',
			visible : ${pt:hasRight(curUser,'delete', entityName2)}
		}, {
			name : 'search_button',
			visible : false
		}, {
	...
	codeCatalogPanel.on("rowclick", function(gridPanel, rowIndex, e) {
		...
		//重新加载下面gird的数据 
		codeLibrary.reloadGrid(encodeURI("codeCatalog='" + key_value + "'"));
	})
</script>
</head>
<body>
